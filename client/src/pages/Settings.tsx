...
export default function Settings({onBack}: SettingsProps) {
    ...
    const removeSub = async (id: string) => {
        ...
        if (isNaN(numeric)) {
            console.warn('removeSub: not a numeric id', id);
            return;
        }
        try {
    ...
    const editSub = async (id: string, newName: string ) => {
            ...
            <div className="mx-auto max-w-5xl space-y-12 px-4 py-12 sm:px-6 lg:px-8">
                ...
                <div className="mb-10 flex items-center justify-between gap-4">
                    ...
                    <Button onClick={onBack}
                            className="h-9 transform bg-gradient-to-r from-purple-500 via-indigo-500 to-blue-500 px-4 text-sm text-white shadow transition hover:scale-105 hover:brightness-110"><ArrowLeft
                        className="mr-2 h-4 w-4"/>Back to Dashboard</Button>
                </div>
                ...
                {categories.map(cat => (
                    <div key={cat.id} className="relative mb-14">
                        <div aria-hidden
                             ...
                             style={{background: `radial-gradient(circle at 30% 30%, ${cat.color}AA 0%, transparent 70%)`}}/>
                        <div aria-hidden
                        ...
                        <div
                            ...
                            <div className="rounded-xl bg-white/80 p-4 backdrop-blur-md">
                                <div className="flex items-end gap-3">
                                    <div className="flex-1"><label className="mb-1 block text-sm font-medium">Add New
                                        Subcategory</label><Input placeholder="e.g., Leadership"
                                                                  value={newSub[cat.id] ?? ''}
                                                                  onChange={e => setNewSub(p => ({
                                                                      ...p,
                                                                      [cat.id]: e.target.value
                                                                  }))}
                                                                  onKeyDown={e => e.key === 'Enter' && addSub(cat.id as GroupId)}/>
                                    </div>
                                    <Button disabled={!newSub[cat.id]?.trim()} onClick={() => addSub(cat.id as GroupId)}
                                            style={{backgroundColor: cat.color}} className="text-white"><Plus
                                        className="mr-2 h-4 w-4"/>Add</Button></div>
                            </div>

                            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">{cat.subcategories.map(sub => (
                                <div key={sub.id}
